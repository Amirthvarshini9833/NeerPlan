...
export default async function AssessmentReportPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="report-page">
    ...
    <article className="report-sheet">
      ...
      <section className="government-schemes" aria-labelledby="government-schemes-heading"><p className="eyebrow">GOVERNMENT SCHEMES &amp; SUPPORT</p><h2 id="government-schemes-heading">Official support routes to check</h2><p className="scheme-intro">These official programmes and resources are matched to <span data-no-translate>{assessment.state}</span>. They are not an approval or guarantee of funding.</p><div className="scheme-list">{governmentSchemes.map((scheme) => <article key={scheme.officialUrl}><p className="scheme-authority">{scheme.authority}</p><h3>{scheme.title}</h3><p>{scheme.summary}</p><p><strong>Who should check:</strong> {scheme.eligibility}</p><a href={scheme.officialUrl} target="_blank" rel="noreferrer">View official information ↗</a><small>Source checked: {new Intl.DateTimeFormat("en-IN", { dateStyle: "medium" }).format(new Date(`${scheme.verifiedOn}T00:00:00`))}</small></article>)}</div><p className="scheme-disclaimer">Government schemes, rules, budgets, and eligibility can change. Verify the latest details and obtain required local permissions before spending money or relying on assistance.</p></section>
